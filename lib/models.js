...
// Record one view of a product, rolled up into today's per-product counter.
// Silently ignores unknown ids so a stale client can't create orphan rows.
function recordProductView(productId) {
  ...
  db.prepare(
    `INSERT INTO product_views (product_id, day, count)
     VALUES (?, date('now', 'localtime'), 1)
     ON CONFLICT(product_id, day) DO UPDATE SET count = count + 1`
  ).run(id);
  return true;
}

// Headline view metrics for the analytics dashboard.
function getViewsAnalytics() {
  const one = (sql) => db.prepare(sql).get();
  const total = one(`SELECT COALESCE(SUM(count), 0) AS v FROM product_views`).v;
  const today = one(`SELECT COALESCE(SUM(count), 0) AS v FROM product_views WHERE day = date('now', 'localtime')`).v;
  const last7 = one(`SELECT COALESCE(SUM(count), 0) AS v FROM product_views WHERE day >= date('now', 'localtime', '-6 days')`).v;
  const last30 = one(`SELECT COALESCE(SUM(count), 0) AS v FROM product_views WHERE day >= date('now', 'localtime', '-29 days')`).v;
  return { totalViews: total, viewsToday: today, views7d: last7, views30d: last30 };
}

// Daily product views for the last `days` (zero-filled, oldest → newest).
function viewsDaily(days = 14) {
  return db
    .prepare(
      `WITH RECURSIVE days(day, idx) AS (
       ...
       SELECT
         days.day AS date,
         CAST(strftime('%m', days.day) AS INTEGER) || '/' || CAST(strftime('%d', days.day) AS INTEGER) AS label,
         COALESCE(totals.views, 0) AS views
       FROM days
       LEFT JOIN totals ON totals.day = days.day
       ORDER BY days.day`
    )
...
// Most-viewed products (deleted products fall out via the join).
function topViewedProducts({ limit = 6 } = {}) {
  return db
    .prepare(
