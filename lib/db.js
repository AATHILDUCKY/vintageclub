...
function migrate(db) {
  db.exec(`
    ...
    -- Product page views, rolled up per product per day. Admin-only analytics —
    -- never shown to customers. One row per (product, day) keeps this table small
    -- (bounded by catalogue size × days) instead of growing per pageview.
    ...
    CREATE INDEX IF NOT EXISTS idx_product_views_day  ON product_views(day);
